package com.github.nirmalpatidar123.imageeffects;
...
public class ImageFilters {
	...
	public Bitmap applyGammaEffect(Bitmap src) {
		return applyGammaEffect(src, 1.8, 1.8, 1.8);
	}
	 ...
	 * @return
	 */
	public Bitmap applyGammaEffect(Bitmap src, double red, double green,
			double blue) {
		// create output image
		Bitmap bmOut = Bitmap.createBitmap(src.getWidth(), src.getHeight(),
				src.getConfig());
	...
	public Bitmap applyRedColorFilterEffect(Bitmap src) {
		return applyColorFilterEffect(src, 255, 0, 0);
	}

	public Bitmap applyGreenColorFilterEffect(Bitmap src) {
		return applyColorFilterEffect(src, 0, 255, 0);
	}

	public Bitmap applyBlueColorFilterEffect(Bitmap src) {
		return applyColorFilterEffect(src, 0, 0, 255);
	}
     ...
     * @return
     */
	public Bitmap applyColorFilterEffect(Bitmap src, double red, double green,
			double blue) {
		// image size
		int width = src.getWidth();
		int height = src.getHeight();
	...
	public Bitmap applySepiaToningEffect(Bitmap src) {
		return applySepiaToningEffect(src, 10, 1.5, 0.6, 0.12);
	}

	public Bitmap applySepiaGreenToningEffect(Bitmap src) {
		return applySepiaToningEffect(src, 10, 0.88, 2.45, 1.43);
	}

	public Bitmap applySepiaBlueToningEffect(Bitmap src) {
		return applySepiaToningEffect(src, 10, 1.2, 0.87, 2.1);
	}
     ...
     * @return
     */
	public Bitmap applySepiaToningEffect(Bitmap src, int depth, double red,
	...
	public Bitmap applyDecreaseColorDepthEffectHigh(Bitmap src) {
		return applyDecreaseColorDepthEffect(src, 64);
	}

	public Bitmap applyDecreaseColorDepthEffectLow(Bitmap src) {
		return applyDecreaseColorDepthEffect(src, 32);
	}
	 ...
	 * @return
	 */
	public Bitmap applyContrastEffect(Bitmap src) {
		return applyContrastEffect(src, 70);
	}
	...
	public Bitmap applyBrightnessEffect(Bitmap src) {
		return applyBrightnessEffect(src, 80);
	}
	...
	// return applySharpenEffect(src, weight);
	// }
	...
	public Bitmap applySmoothEffect(Bitmap src) {
		return applySmoothEffect(src, 100);
	}
	...
	public Bitmap applyBoostEffect(Bitmap src) {
		return applyBoostEffect(src, 2, 30);
	}
	...
	public Bitmap applyRoundCornerEffect(Bitmap src) {
		return applyRoundCornerEffect(src, 45);
	}
	...
	public Bitmap applyWaterMarkEffect(Bitmap src){
		return applyWaterMarkEffect(src, "WaterMark", 200, 200, Color.GREEN, 80, 24, false);
	}
	...
	public Bitmap applyTintEffect(Bitmap src) {
		return applyTintEffect(src, 100);
	}
	...
	public Bitmap applyShadingFilter(Bitmap source){
		return applyShadingFilter(source, Color.CYAN);
	}
	...
	public Bitmap applySaturationFilter(Bitmap source){
		return applySaturationFilter(source, 1);
	}
	/**
	 * 
	 * @param source
	...
	public Bitmap applyHueFilter(Bitmap source) {
		return applyHueFilter(source, 2);
	}
